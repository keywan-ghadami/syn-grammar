...
/// `S` is an optional state a backend can pass along; the syn path does not
/// use it and leaves it at `()`.
...
impl<T: Debug, E: Display + Debug, S> TestResult<T, E, S> {
    ...
    ///
    /// Only for output. The `assert_failure_*` assertions deliberately compare
...
impl<E: Display + Debug, S> TestResult<f32, E, S> {
    ...
    ///
    /// The same limitation as for the `f64` version.
...
pub trait Testable<T, E> {
    /// Lifts `self` into a `TestResult` to attach assertions.
