...
mod demo {
    ...
    grammar Demo {
        ...
        pub rule assignment -> i32 = "let" name:ident "=" v:value ";" -> {
            let _ = name;
            v
        }

        rule value -> i32 = i:i32 -> { i }

        /// Only for the adjacency case: `::` must be a joint operator.
        /// `a : : b` must NOT match.
        pub rule path -> String = a:ident "::" b:ident -> {
            format!("{}::{}", a, b)
        }
    }
    }
...
pub fn assignment(input: TokenStream) -> TokenStream {
    run(input, Demo::parse_assignment)
}

/// Parses `a::b` and returns the path as a string literal.
#[proc_macro]
pub fn path(input: TokenStream) -> TokenStream {
    run(input, |s| Demo::parse_path(s).map(|p| p.len() as i32))
}
...
fn run<F>(input: TokenStream, parser: F) -> TokenStream
where
