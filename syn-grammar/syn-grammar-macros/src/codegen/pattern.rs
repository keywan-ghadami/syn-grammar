...
pub fn generate_sequence(patterns: &[ModelPattern], action: &TokenStream, ctx: &CodegenContext) -> Result<TokenStream> {
...
pub fn generate_sequence_steps(patterns: &[ModelPattern], ctx: &CodegenContext) -> Result<TokenStream> {
    let mut steps = Vec::new();
    for p in patterns { steps.push(generate_pattern_step(p, ctx)?); }
    Ok(quote! { #(#steps)* })
}
    ...
    match pattern {
        ...
        ModelPattern::Lit { binding, lit } => {
            if let Lit::Str(lit) = lit {
                ...
                if token_types.len() <= 1 {
                    ...
                    let bind_stmt = if let Some(bind) = binding { quote! { let #bind = ( #(#results),* ); } } else { quote! {} };
                    Ok(quote! { #(#steps)* #(#checks)* #bind_stmt })
                }
            } else {
                Err(syn::Error::new(lit.span(), "Non-string literals are not supported as matchers."))
            }
        }
        ModelPattern::RuleCall { binding, rule_path, generics, args } => {
            let rule_name_ident = rule_path.get_ident();
            let is_builtin = rule_name_ident.is_some_and(|ident| SynBackend::get_builtins().iter().any(|b| ident == b.name));
            let is_syn_type = rule_path.segments.first().map_or(false, |seg| seg.ident == "syn");

            if is_syn_type {
                let bind_stmt = if let Some(bind) = binding { quote!(let #bind = _val;) } else { quote!() };
                Ok(quote! {
                ...
                let rule_arg = match &args[0] { Argument::Positional(p) | Argument::Named(_, p) => p };
                let sep_arg = match &args[1] { Argument::Positional(p) | Argument::Named(_, p) => p };
                let mut min = 0usize;
                ...
                for arg in &args[2..] {
                    match arg {
                        Argument::Named(id, val) => {
                            if id == "min" { if let ModelPattern::Lit { lit: Lit::Int(i), .. } = val { min = i.base10_parse()?; } }
                            else if id == "trailing" { if let ModelPattern::Lit { lit: Lit::Bool(b), .. } = val { trailing = b.value; } }
                            else if id == "item_label" { if let ModelPattern::Lit { lit: Lit::Str(s), .. } = val { item_label = Some(s.value()); } }
                        }
                        Argument::Positional(val) => { if let ModelPattern::Lit { lit: Lit::Int(i), .. } = val { min = i.base10_parse()?; } }
                    }
                }

                let container_ty = generics.first().map_or(quote!(Vec), |ty| match ty { syn::Type::Infer(_) => quote!(Vec), _ => quote!(#ty) });
                let (rule_arg_with_binding, item_binding) = match rule_arg {
                    ModelPattern::RuleCall { binding: None, rule_path, generics, args } => {
                        ...
                        (ModelPattern::RuleCall { binding: Some(temp.clone()), rule_path: rule_path.clone(), generics: generics.clone(), args: args.clone() }, vec![temp])
                    }
                    ModelPattern::Lit { binding: None, lit } => {
                        let temp = format_ident!("_item");
                        (ModelPattern::Lit { binding: Some(temp.clone()), lit: lit.clone() }, vec![temp])
                    }
                    _ => (rule_arg.clone(), analysis::collect_bindings(std::slice::from_ref(rule_arg))),
                };

                let rule_parser = generate_pattern_step(&rule_arg_with_binding, ctx)?;
                let sep_parser = generate_pattern_step(sep_arg, ctx)?;
                let item_return_expr = if item_binding.len() == 1 { let b = &item_binding[0]; quote!(#b) } else if item_binding.is_empty() { quote!(()) } else { quote!((#(#item_binding),*)) };
                let label_tokens = if let Some(l) = item_label { quote!(#l) } else { quote!("item") };

                let bind_stmt = if let Some(bind) = binding { quote!(let #bind = #container_ty::from_iter(_items_vec);) } else { quote!() };
                ...
                let rule_parser = generate_pattern_step(&rule_arg_with_binding, ctx)?;
                let item_return_expr = if item_binding.len() == 1 { let b = &item_binding[0]; quote!(#b) } else if item_binding.is_empty() { quote!(()) } else { quote!((#(#item_binding),*)) };
                let item_label_expr = if let Some(msg) = item_label { quote!(#msg) } else { quote!("item") };
        ...
        ModelPattern::Repeat(inner, _) => {
            ...
            if !bindings.is_empty() {
                let vec_names: Vec<_> = bindings.iter().map(|b| format_ident!("_vec_{}", b)).collect();
                let init_vecs: Vec<_> = vec_names.iter().map(|v| quote!(let mut #v = Vec::new();)).collect();
                let push_vecs: Vec<_> = vec_names.iter().zip(bindings.iter()).map(|(v, b)| quote!(#v.push(#b);)).collect();
                let finalize_vecs: Vec<_> = bindings.iter().zip(vec_names.iter()).map(|(b, v)| quote!(let #b = #v;)).collect();
        ...
        ModelPattern::Group { binding, alts, .. } => {
            ...
            let temp_variants = alts.iter().map(|(pat_seq, action, label)| {
                let bindings = analysis::collect_bindings(pat_seq);
                let action_expr = if let Some(a) = action { quote!({ #a }) } else if bindings.is_empty() { quote!(()) } else if bindings.len() == 1 { let b = &bindings[0]; quote!(#b) } else { quote!(( #(#bindings),* )) };
                RuleVariant { pattern: pat_seq.clone(), label: label.clone(), action: quote!({ #action_expr }), with_span: false, is_explicit: false }
            }).collect::<Vec<_>>();
        ...
        ModelPattern::Bracketed(s, _) | ModelPattern::Braced(s, _) | ModelPattern::Parenthesized(s, _) => {
            let delimiter = match pattern { ModelPattern::Bracketed(_, _) => quote!(Bracket), ModelPattern::Braced(_, _) => quote!(Brace), _ => quote!(Parenthesis) };
        ...
        ModelPattern::Until { binding, pattern, .. } => {
            ...
            let bind_stmt = if let Some(bind) = binding { quote!(let #bind = _tokens_stream;) } else { quote!() };
            Ok(quote! {
...
fn generate_rule_call_expr(rule_path: &syn::Path, args: &[Argument], ctx: &CodegenContext) -> Result<TokenStream> {
    ...
    for arg in args {
        match arg {
            Argument::Positional(p) | Argument::Named(_, p) => match p {
                ...
                ModelPattern::RuleCall { rule_path, args, .. } if args.is_empty() && rule_path.get_ident().is_some() => {
                    let ident = rule_path.get_ident().unwrap();
                    arg_exprs.push(quote!(#ident));
                }
                _ => return Ok(quote!(compile_error!("Complex pattern used as runtime argument"))),
            },
        }
    }
    ...
    if let Some(ident) = rule_path.get_ident() {
        if ctx.grammar.extern_rules.iter().any(|er| er.name == *ident) {
            Ok(if arg_exprs.is_empty() { quote!(#rule_path(cursor)?) } else { quote!(#rule_path(cursor, #(#arg_exprs),*)?) })
        } else if ctx.grammar.rules.iter().any(|r| r.name == *ident) {
            let impl_name = format_ident!("parse_{}_impl", ident);
            Ok(if arg_exprs.is_empty() { quote!(#impl_name(cursor, ctx)?) } else { quote!(#impl_name(cursor, ctx, #(#arg_exprs),*)?) })
        } else {
            Ok(if arg_exprs.is_empty() { quote!(#rule_path(cursor)?) } else { quote!(#rule_path(cursor, #(#arg_exprs),*)?) })
        }
    } else {
        ...
        if is_import_alias {
            ...
            Ok(if arg_exprs.is_empty() { quote!(#new_path(cursor, ctx)?) } else { quote!(#new_path(cursor, ctx, #(#arg_exprs),*)?) })
        } else {
            Ok(if arg_exprs.is_empty() { quote!(#rule_path(cursor)?) } else { quote!(#rule_path(cursor, #(#arg_exprs),*)?) })
        }
    }
