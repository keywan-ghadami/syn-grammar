...
impl Monomorphizer {
    ...
    fn instantiate(&mut self, template: &Rule, args: &[ModelPattern]) -> Ident {
        let args_repr = args.iter().map(|a| format!("{:?}", a)).collect::<Vec<_>>().join(",");
        ...
        let param_map: HashMap<Ident, ModelPattern> = grammar_params.iter().zip(args.iter()).map(|(k, v)| (k.clone(), v.clone())).collect();
        ...
        let substituter = ParamSubstituter { param_map: &param_map };
        ...
        let generic_params: Vec<Ident> = old_generics.type_params().map(|tp| tp.ident.clone()).collect();
        ...
        let mut type_substituter = TypeSubstituter { type_map: &type_map };
        ...
        self.rule_types.insert(new_name.clone(), new_rule.return_type.clone());
    ...
    fn infer_type(&self, pattern: &ModelPattern) -> Option<Type> {
        match pattern {
            ...
            ModelPattern::RuleCall { rule_name, .. } => {
                self.rule_types.get(rule_name).cloned()
            }
            _ => None,
        }
    }
                 ...
                 if let Some(replacement) = self.param_map.get(rule_name) {
                     *pattern = replacement.clone();
                     ...
                     if let Some(b) = old_binding {
                         match pattern {
                             ModelPattern::RuleCall { binding: ref mut new_b, .. } => {
                                 if new_b.is_none() {
                                     *new_b = Some(b);
                                 }
                             }
                             ModelPattern::Recover { binding: ref mut new_b, .. } => {
                                 if new_b.is_none() {
                                     *new_b = Some(b);
                                 }
                             }
