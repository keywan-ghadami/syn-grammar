...
## Decision
We will implement a **Namespaced Grammar Mixin Pattern** based on macro push-down accumulation. 
...
For every defined `grammar!` (e.g., `grammar Base { ... }`), the backend will not only generate the final Rust module but *additionally* emit a declarative macro (`macro_rules! Base_rules`). This macro serves as the carrier for the raw token stream of that grammar.

### 2. Syntactic Sugar & Push-Down Accumulation
The frontend macro (`grammar!`) will accept a new, flat include syntax with an explicit alias:
```rust
grammar! {
    include Whitespace_rules as w;
        include Math_rules as m;
                ...
                * **Model (`syn-grammar-model`):** The AST (specifically the `RuleCall` node) will be expanded to store paths instead of simple identifiers (`RuleReference { namespace: Option<Ident>, name: Ident }`).
                * **Codegen:** When compiling into flat Rust functions, deterministic name mangling using a double underscore is applied: the call `w::ws` is translated into the function signature `parse_w__ws(input)`.
                ...
                ### Positive
                * **Perfect Spans:** Since raw token streams (including their original metadata) are passed through, IDE error messages for syntax or shadowing errors will point exactly to the correct line in the referenced source file.
